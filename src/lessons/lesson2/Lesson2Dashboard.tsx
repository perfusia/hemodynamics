...
function FrankStarlingChart({ preload, contractility, afterload }: {
  ...
  return (
    <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 'auto' }}>
      {yTicks.map(t => <line key={t} x1={padL} x2={W - padR} y1={py(t)} y2={py(t)} stroke="rgba(255,255,255,0.05)" strokeWidth="1" />)}
      {xTicks.map(t => <line key={t} x1={px(t)} x2={px(t)} y1={padT} y2={H - padB} stroke="rgba(255,255,255,0.05)" strokeWidth="1" />)}
      <rect x={px(N.preload.low)} width={px(N.preload.high) - px(N.preload.low)} y={padT} height={innerH} fill="rgba(52,211,153,0.04)" />
      <line x1={padL} x2={padL} y1={padT} y2={H - padB} stroke="rgba(255,255,255,0.15)" strokeWidth="1" />
      <line x1={padL} x2={W - padR} y1={H - padB} y2={H - padB} stroke="rgba(255,255,255,0.15)" strokeWidth="1" />
      {xTicks.map(t => <text key={t} x={px(t)} y={H - 8} textAnchor="middle" fill="rgba(255,255,255,0.2)" fontSize="9" fontFamily="ui-monospace,monospace">{t}</text>)}
      {yTicks.filter(t => t > 0).map(t => <text key={t} x={padL - 5} y={py(t) + 3} textAnchor="end" fill="rgba(255,255,255,0.2)" fontSize="9" fontFamily="ui-monospace,monospace">{t}</text>)}
      <text x={padL + innerW / 2} y={H - 1} textAnchor="middle" fill="rgba(255,255,255,0.25)" fontSize="9" fontFamily="ui-sans-serif,sans-serif">Preload (mmHg)</text>
      <text x={10} y={padT + innerH / 2} textAnchor="middle" fill="rgba(255,255,255,0.25)" fontSize="9" fontFamily="ui-sans-serif,sans-serif" transform={`rotate(-90, 10, ${padT + innerH / 2})`}>SV (mL)</text>
      <path d={toPath(normalCurve)} fill="none" stroke="rgba(255,255,255,0.1)" strokeWidth="1.5" strokeDasharray="4 3" />
      <path d={toPath(curve)} fill="none" stroke="#34d399" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
      <line x1={padL} x2={W - padR} y1={py(N.sv.low)} y2={py(N.sv.low)} stroke="rgba(52,211,153,0.2)" strokeWidth="1" strokeDasharray="3 3" />
      <line x1={padL} x2={W - padR} y1={py(N.sv.high)} y2={py(N.sv.high)} stroke="rgba(52,211,153,0.2)" strokeWidth="1" strokeDasharray="3 3" />
      <line x1={dotX} x2={dotX} y1={padT} y2={H - padB} stroke="rgba(255,255,255,0.12)" strokeWidth="1" strokeDasharray="3 3" />
      <line x1={padL} x2={W - padR} y1={dotY} y2={dotY} stroke="rgba(255,255,255,0.12)" strokeWidth="1" strokeDasharray="3 3" />
      <circle cx={dotX} cy={dotY} r={5} fill="#ffffff" />
      <circle cx={dotX} cy={dotY} r={8} fill="none" stroke="rgba(255,255,255,0.3)" strokeWidth="1.5" />
      <text x={Math.min(dotX + 10, W - padR - 30)} y={dotY - 5} fill="white" fontSize="9" fontFamily="ui-monospace,monospace">{currentSV} mL</text>
    </svg>
  )
}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1, minWidth: 120 }}>
            ...
            <div style={{ fontFamily: 'ui-monospace,monospace', fontSize: 10, marginTop: 7, color: 'rgba(255,255,255,0.25)' }}>
              <span style={{ color: '#86efac' }}>{hr}</span>
              <span style={{ color: 'rgba(255,255,255,0.2)' }}> × </span>
              <span style={{ color: '#93c5fd' }}>{sv}</span>
              <span style={{ color: 'rgba(255,255,255,0.2)' }}> = </span>
              <span style={{ color: 'white' }}>{co} L/min → </span>
      ...
      <div style={card}>
        ...
        <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.25)', lineHeight: 1.6, marginTop: 8, marginBottom: 0 }}>
  ...
  return (
    <div>
      ...
      <section style={{ marginBottom: 52 }}>
        ...
        <P><strong style={{ color: 'white', fontWeight: 500 }}>Preload</strong> is how much the ventricle is stretched before it contracts — like a rubber band. More stretch, more force. Determined primarily by venous return. A dehydrated patient has low preload and low stroke volume. IV fluid increases preload and stroke volume — up to a point.</P>
        <P><strong style={{ color: 'white', fontWeight: 500 }}>Afterload</strong> is the resistance the ventricle must overcome to eject blood — essentially SVR from the heart's perspective. High afterload means harder squeezing, less ejection. This is why vasodilators can actually improve stroke volume in a failing heart even though they lower blood pressure.</P>
        <P><strong style={{ color: 'white', fontWeight: 500 }}>Contractility</strong> is the intrinsic strength of the heart muscle independent of preload and afterload. Reduced in heart failure, after MI, acidosis, or hypoxia. Increased by catecholamines and inotropes.</P>
        <Callout label="Try it in the simulator">
      ...
      <section style={{ marginBottom: 52 }}>
        ...
        <P>This curve answers the most common ICU question: <em style={{ color: 'rgba(255,255,255,0.7)' }}>should I give this patient more fluid?</em> Steep ascending limb — yes, fluid helps. Flat or descending limb — no, fluid causes pulmonary edema.</P>
        <Callout label="Try it in the simulator">
  ...
  return (
    <div>
      <div style={{ marginBottom: 28 }}>
        <div style={{ fontFamily: 'ui-monospace,monospace', fontSize: 10, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'rgba(52,211,153,0.6)', marginBottom: 8 }}>Lesson 2</div>
