...
import { MONITOR, STATUS, STATUS_MON } from '../../shared/tokens'
...
function RangeTable({ rows }: { rows: { range: string; label: string; color: string; note: string }[] }) {
  return (
    <div style={{ margin: '18px 0', borderRadius: 10, border: '1px solid #C7D2C4', overflow: 'hidden' }}>
      {rows.map((row, i) => (
        <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '9px 14px', borderBottom: i < rows.length - 1 ? '1px solid #C7D2C4' : 'none', background: 'rgba(22,33,28,0.020)', flexWrap: 'wrap' }}>
          ...
          <span style={{ fontSize: 11, color: 'rgba(22,33,28,0.63)' }}>{row.note}</span>
        </div>
      ))}
    </div>
  )
...
function Simulator() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {/* MAP card */}
      <div style={{ ...card, background: MONITOR.bg, borderColor: '#000', padding: '16px 18px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
          <MAPArc map={map} status={status} />
          <div style={{ flex: 1, minWidth: 120 }}>
            <div style={{ fontSize: 9, fontFamily: 'var(--font-mono)', letterSpacing: '0.1em', textTransform: 'uppercase', color: MONITOR.label, marginBottom: 4 }}>Mean Arterial Pressure</div>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
              <span style={{ fontSize: 40, fontWeight: 600, fontFamily: 'var(--font-mono)', color: MONITOR.art, lineHeight: 1, fontVariantNumeric: 'tabular-nums' }}>{map}</span>
              <span style={{ fontSize: 13, color: MONITOR.label }}>mmHg</span>
            </div>
            <div style={{ fontSize: 12, fontWeight: 600, color: status.accent, marginTop: 2 }}>{status.label}</div>
            <div style={{ fontSize: 11, color: MONITOR.label, marginTop: 1 }}>{status.sub}</div>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, marginTop: 7, color: MONITOR.label }}>
              <span style={{ color: MONITOR.cvp }}>{co.toFixed(1)}</span>
              <span style={{ color: MONITOR.label }}> × </span>
              <span style={{ color: MONITOR.pa }}>{svr}</span>
              <span style={{ color: MONITOR.label }}> ÷ 80 = </span>
            ...
            {[
              ...
              return (
                <div key={s.abbr} style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
                  <span style={{ fontSize: 9, fontFamily: 'var(--font-mono)', color: MONITOR.label, width: 22 }}>{s.abbr}</span>
                  <span style={{ fontSize: 13, fontFamily: 'var(--font-mono)', fontWeight: 500, fontVariantNumeric: 'tabular-nums', color: st.color === STATUS.normal ? STATUS_MON.normal : st.color === STATUS.warn ? STATUS_MON.warn : STATUS_MON.critical }}>{s.v}</span>
                </div>
              )
            })}
          </div>
      ...
      <div style={card}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
          ...
          {activeDrug && <span style={{ fontSize: 10, fontFamily: 'var(--font-mono)', color: 'rgba(147,197,253,0.65)' }}>{activeDrug} — tap to remove</span>}
        </div>
        <p style={{ fontSize: 11, color: 'rgba(22,33,28,0.5)', lineHeight: 1.6, marginTop: -8, marginBottom: 12 }}>
  ...
  return (
    <div>
      ...
      <section style={{ marginBottom: 52 }}>
        ...
        <P>What matters is <strong style={{ color: '#16211C', fontWeight: 500 }}>Mean Arterial Pressure (MAP)</strong> — the average pressure delivered to your organs continuously throughout the entire cardiac cycle. Your kidneys, brain, and gut experience a continuous pressure, and that pressure is what keeps them alive.</P>
        <Eq>MAP = DBP + ¹⁄₃(SBP − DBP)</Eq>
        <P>For 120/80: MAP = 80 + (40/3) ≈ <strong style={{ color: '#16211C' }}>93 mmHg</strong>. Normal. Good perfusion.</P>
        <P>The single most important threshold in critical care is <strong style={{ color: '#A32319' }}>65 mmHg</strong>. Below this, kidneys, gut, and brain begin to lose adequate blood flow.</P>
        <RangeTable rows={[
          { range: '< 65 mmHg',    label: 'Dangerous', color: '#A32319', note: 'Organ perfusion at risk — intervene immediately' },
          { range: '65–70 mmHg',   label: 'Low',       color: '#96590A', note: 'Close to threshold — monitor closely' },
          { range: '70–100 mmHg',  label: 'Normal',    color: '#2E6B4F', note: 'Organs adequately perfused' },
          { range: '101–120 mmHg', label: 'Elevated',  color: '#96590A', note: 'Sustained hypertension' },
          { range: '> 120 mmHg',   label: 'Crisis',    color: '#A32319', note: 'Hypertensive emergency' },
        ]} />
      ...
      <section style={{ marginBottom: 52 }}>
        ...
        <Eq>MAP = CO × SVR</Eq>
        <P><strong style={{ color: '#16211C', fontWeight: 500 }}>CO (Cardiac Output)</strong> is the total volume of blood your heart pumps per minute, in L/min. Normal resting: 4–8 L/min.</P>
        <P><strong style={{ color: '#16211C', fontWeight: 500 }}>SVR (Systemic Vascular Resistance)</strong> is the collective resistance of all blood vessels. Think of a garden hose — pinch the end, pressure builds. Normal: 800–1200 dynes·sec/cm⁵. Higher means tighter vessels.</P>
      ...
      <section style={{ marginBottom: 52 }}>
        ...
        <P><strong style={{ color: '#16211C', fontWeight: 500 }}>Septic shock</strong> — SVR crashes from vasodilation. Treat with vasopressors.</P>
        <P><strong style={{ color: '#16211C', fontWeight: 500 }}>Cardiogenic shock</strong> — CO falls from a damaged heart. Treat with inotropes, not vasopressors.</P>
        <P><strong style={{ color: '#16211C', fontWeight: 500 }}>Anesthesia induction</strong> — agents vasodilate, SVR drops, MAP falls. Predictable and manageable.</P>
      ...
      <section style={{ marginBottom: 52 }}>
        ...
        <Eq>CO = HR × SV</Eq>
        <P><strong style={{ color: '#16211C', fontWeight: 500 }}>HR</strong> (Heart Rate): 60–100 bpm normal. <strong style={{ color: '#16211C', fontWeight: 500 }}>SV</strong> (Stroke Volume): 60–100 mL/beat normal.</P>
        <P>70 bpm × 70 mL/beat = 4,900 mL/min ≈ <strong style={{ color: '#16211C' }}>5 L/min</strong>. Textbook normal cardiac output.</P>
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 28 }}>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'rgba(46,107,79,0.6)', marginBottom: 8 }}>Lesson 1</div>
